...
## 2026-06-02 추가 기록: 최종 엔딩 검은 화면 / 저장 후 재접속 쓰레기 리스폰 보강

수정한 문제:
- 에필로그 엄마 전화 후 최종 엔딩 이미지로 넘어갈 때 검은 화면이 유지될 수 있는 문제를 보강했습니다.
- 설정창의 “게임 저장하고 나가기” 후 이어하기를 했을 때, 초기 쓰레기 생성 순서와 체크포인트 복원 순서가 어긋나 화면에 쓰레기가 없는 상태로 시작할 수 있는 문제를 보강했습니다.
...
변경 내용:
- `TravelEndingSystem`에 최종 엔딩 중복 진입 방지 가드(`isFinalEndingStarting`)를 추가했습니다.
- 엄마 전화 선택지 표시 시점의 선행 최종 이미지 로딩을 제거했습니다. 최종 이미지 로딩은 실제 최종 엔딩 진입 시점에만 처리합니다.
- `loadFinalEndingTexture()`를 추가해 최종 이미지가 이미 있으면 즉시 표시하고, 없으면 로더 완료를 기다리도록 했습니다.
- 로더가 이미 실행 중인 경우에는 기존 로딩 완료 후 다시 확인하고, 필요할 때만 최종 이미지를 큐에 넣습니다.
- 3.5초 fallback 타이머를 두어 이미지 로딩 실패나 지연이 있어도 반드시 `fadeIn()`까지 진행되게 했습니다.
- 최종 이미지가 준비되지 못하면 `showFinalEndingFallback()`으로 간단한 챕터 완료 화면을 표시합니다.
- 일반 엄마 전화 종료 fadeOut 경로는 `FADE_OUT_COMPLETE` 리스너를 먼저 등록한 뒤 fadeOut을 시작하도록 순서를 안정화했습니다.
- `SlimeSystem.ensureActiveTrash()`를 추가해 튜토리얼 완료 상태에서 쓰레기가 너무 적으면 즉시 보충하게 했습니다.
- `PlayScene`은 체크포인트 복원 후 `tutorialState === "completed"`일 때 리스폰 루프 시작과 `ensureActiveTrash()` 호출만 담당합니다.

원인 메모:
- 검은 화면은 최종 이미지 로딩과 카메라 fade 상태가 엇갈릴 때 `fadeIn()`이 호출되지 않아 남을 가능성이 컸습니다.
- 저장 후 재접속 리스폰 문제는 `spawnTrashWave()`가 체크포인트 복원보다 먼저 실행되고, 그 시점의 `tutorialState`가 아직 `intro`라 쓰레기 생성이 스킵되는 순서와 관련이 있습니다. 복원 후 타이머만 켜면 첫 리스폰까지 기다려야 하므로, 복원 직후 최소 수량을 즉시 보장하도록 보강했습니다.

검증:
- `node --check src/systems/TravelEndingSystem.js`: 통과
- `node --check src/systems/SlimeSystem.js`: 통과
- `node --check src/scenes/PlayScene.js`: 통과

다음 확인:
- 실제 플레이에서 엄마 전화 수락 후 최종 엔딩 이미지가 정상 표시되는지 확인해야 합니다.
- 이어하기 직후 쓰레기가 즉시 맵에 보이는지 확인해야 합니다.

---

## 2026-06-02 추가 기록: 여비 보상 구조 변경 / 프롤로그 축소 검토

이번 변경:
- 여비 캔 모으기 퀘스트 보상을 `10,000원`에서 `빗자루`로 변경했습니다.
- 여비 분리수거 퀘스트 완료 보상은 `분리수거장 해금`만 남겼습니다.
- 분리수거 완료 대사는 빗자루 지급 문맥을 제거하고, "치우면 100원 + 맞게 분리수거하면 100원" 구조를 설명하도록 바꿨습니다.
- 기존 저장 파일 호환을 위해, 이미 캔 퀘스트를 완료했지만 빗자루를 받지 못한 세이브는 이어하기 시 빗자루가 드롭되도록 보강했습니다.
...
보상 설계 평가:
- 이 방향은 좋습니다. 돈 보상은 반복 청소/분리수거 루프에서 얻고, 퀘스트 보상은 도구/해금/경험으로 주는 구조가 더 선명합니다.
- 캔 모으기 완료: "더 잘 치울 수 있는 도구를 얻는다"는 즉각적인 성장 보상.
- 분리수거 완료: "분리수거장을 사용할 수 있다"는 장기 반복 시스템 해금.
- 수니수니 보상도 돈 대신 활력수 중심으로 바뀐 흐름과 일관됩니다.

검증:
- `node --check src/systems/YebiQuestSystem.js`: 통과
- `node --check src/systems/CheckpointStorage.js`: 통과

프롤로그 축소 의견:
- 아직 코드 수정은 하지 않았습니다.
- 반복 플레이 기준으로 프롤로그가 조금 루즈하게 느껴질 수 있다는 의견에 동의합니다.
- 다만 첫 플레이에서는 동기 부여가 중요하므로 완전 삭제보다는 "대사 수 축소 + 장면 전환 유지"가 좋습니다.
- 추천 방향:
  1. 친구 전화: 여행 제안과 돈 필요성을 빠르게 보여주는 2~3줄만 유지.
  2. 잔고 확인: 돈이 부족하다는 핵심 정보만 짧게 유지.
  3. 엄마 제안: 삼각지 청소로 용돈을 벌 수 있다는 문장 중심으로 압축.
  4. 이동 컷: 분위기 전환용으로 짧게 유지하되 대사는 줄이기.
- 목표는 프롤로그 전체를 약 60~70% 길이로 줄이고, 첫 조작까지 도달 시간을 짧게 만드는 것입니다.
- 다음에 수정한다면 `PrologueScene` 또는 프롤로그 데이터/시퀀스 파일에서 대사 배열을 먼저 줄이고, 연출 타이밍은 마지막에 다듬는 순서가 안전합니다.

현재 핸드오프 기준 남은 주요 확인/후보:
- 최종 엔딩 검은 화면 수정 후 실제 플레이 확인.
- 저장 후 재접속 시 쓰레기 즉시 보충 확인.
- 병원/약국/상점/집/엔딩 등 내부 장면 전환 페이드 시스템화.
- 병원/약국 대사부터 JSON 데이터로 이전.
- `Preload.js` 에셋 목록 분리.
- DOM 오버레이 추가 시 입력 차단/스크롤 체크리스트 계속 적용.

---

## 2026-06-02 추가 기록: 프롤로그 압축 적용

수정한 문제/목표:
- 반복 플레이 기준으로 프롤로그가 조금 루즈하게 느껴질 수 있어, 첫 조작까지 걸리는 시간을 줄였습니다.
- 첫 플레이어에게 필요한 동기인 `서울 여행 제안 -> 돈 부족 -> 엄마의 청소 수당 제안 -> 삼각지 도착` 흐름은 유지했습니다.

수정 파일:
- `src/scenes/PrologueScene.js`

변경 내용:
- 프롤로그 대사량을 약 22줄에서 12줄 수준으로 줄였습니다.
- 방 안 시작, 쭉쭉이 전화, 지갑 확인, 엄마 전화, 이동, 삼각지 도착 배경 전환은 유지했습니다.
- 각 장면의 핵심 정보만 남기고 감탄/반복 설명 문장을 줄였습니다.
- 코드 구조와 장면 함수 분리는 유지했고, PlayScene에는 손대지 않았습니다.

의도:
- 프롤로그를 없애지 않고, 동기 부여는 살리되 템포를 빠르게 만드는 방향입니다.
- 반복 플레이 피로도를 줄이고, 청소 조작에 더 빨리 진입하도록 했습니다.

검증:
- `node --check src/scenes/PrologueScene.js`: 통과

다음 확인:
- 실제 플레이에서 대사 템포가 너무 급하지 않은지 확인하면 좋습니다.
- 필요하면 다음 단계에서 `처음 보기/짧게 보기` 옵션을 시작 화면이나 설정에 추가할 수 있습니다.
