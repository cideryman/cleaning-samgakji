...
- 쭉쭉이 지갑 대화 진입, 자판기 보상 연결, 플로깅 요청, 동행 따라오기, 자동 청소, 동행 종료: `JjookQuestSystem`
...
`JjookQuestSystem`은 현재 지갑 대화 진입, 자판기 보상 연결, 플로깅 요청, 쭉쭉이 동행, 자동 청소, 동행 종료와 콜라 보답을 담당한다. 지갑 아이템 생성/수거, 옷가게, 짐싸기/버스 흐름은 아직 `PlayScene.js`에 남아 있다.
