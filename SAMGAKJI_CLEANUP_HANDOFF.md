...
- 동네 성장 단계 토스트 문구를 "삼각지에 꽃이 피고 있어요/나비가 오고 있어요"처럼 맵 변화가 직접 느껴지는 표현으로 조정했습니다.
...
- 여비 분리수거 퀘스트 수락 직후, 일반쓰레기/캔/플라스틱을 각각 맞는 통에 넣는 3단계 시범 연출을 추가했습니다. 실제 재고나 퀘스트 카운트는 올리지 않고, 연출이 끝난 뒤 `startRecycleQuest()`가 실행됩니다.
- 여비 분리수거 시범은 이제 여비가 왼쪽 통부터 각 통 앞으로 걸어간 뒤 쓰레기 스프라이트가 통으로 들어가는 이동형 연출입니다. 통 앞 대기 위치는 Tiled `spawn` point에 `recycle_demo_normal`, `recycle_demo_can`, `recycle_demo_plastic`을 추가하면 조정할 수 있고, 없으면 각 통 좌표 기준 fallback을 씁니다.
- NPC 말풍선 공통 스타일을 검은 라운드 박스와 흰 글씨로 바꿨고, 랜덤/기억 대사의 표시 시간을 늘렸습니다. 분리수거통 아래 라벨도 라운드 패널로 변경했습니다.
- 편의점 배움 아이콘 좌표를 건물 뒤쪽이 아니라 문 근처로 내렸습니다.
